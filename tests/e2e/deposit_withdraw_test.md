...
| Name | Type | Purpose | Required mapping |
|------|------|---------|------------------|
| `relay_chain_system` | private key | Relay blockchain signer key | Relay `blockchains.dymension.account.address` and `blockchains.near.account.address` MUST be derived from this key. Relay `config/secrets/blockchain_system_private_key.txt` MUST store this key. |
| `relay_wallet_chain_system` | private key | Relay Wallet system payout key | Relay Wallet `blockchains.dymension.account.address` and `blockchains.near.account.address` MUST be derived from this key. Relay Wallet `/app/config/blockchain_privkey.txt` MUST store this key. |
| `relay_wallet_relay_api` | private key | Relay Wallet authentication key for Relay withdraw APIs | Relay Wallet `relay.api.private_key_file` MUST point to the file storing this key. Relay `withdraw.relay_wallet_address` MUST equal the address derived from this key. |
| `relay_credits_api_auth` | private key | Relay credits API authorization signer key | Relay `credits.api_auth_address` MUST equal the address derived from this key. |
...
| `relay_account.deposit_address` | address | Deposit target address | Relay `relay_account.deposit_address` MUST be set to this address. |
| `withdraw.withdrawal_fee_address` | address | Withdrawal fee receiver address | Relay `withdraw.withdrawal_fee_address` MUST be set to this address. |
| `dao.task_fee_share_address` | address | DAO task fee share receiver address | Relay `dao.task_fee_share_address` MUST be set to this address. |
...
- Wait for confirmation and relay ingestion.
...
- Wait for Relay Wallet to process and fulfill the request.
...
- Verify withdrawal record fields, success status, payout transaction hash, and expected balance decrease.
